    ...
    <div v-if="document">
      <div class="overflow-hidden relative">
          ...
          <div class="absolute left-1/2 top-1/2 -translate-x-1/2 px-12 py-6 -translate-y-1/2 bg-white rounded-2xl">
            ...
            <p class="py-5 text-red"> Hosted By <NuxtLink :to="{path: `/profile/${document.userId}`}">{{ document.userName }}</NuxtLink></p>
            ...
            <div v-if="ownership" @click="handleDelete" class="w-full py-2 mt-5 text-center font-bold text-slate-200 rounded-xl cursor-pointer bg-red">Delete Event</div>
          </div>
      </div>
    </div>
  </div>
